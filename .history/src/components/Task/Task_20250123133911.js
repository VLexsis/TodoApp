            ...
            <div className="view">
                ...
                <label>
                    ...
                    <span className="description">
                        {/* Отображаем кнопку "Play" только если задача не активна */}
                        {!isActive && <button className="icon icon-play" onClick={() => onPlayTimer(task.id)}></button>}
                        {/* Отображаем кнопку "Pause" только если задача активна */}
                        {isActive && (
...
Task.defaultProps = {
    ...
    onToggleEdit: () => {},
    onToggleDone: () => {},
    isActive: false, // Добавляем значение по умолчанию для isActive
}
...
Task.propTypes = {
    ...
    isActive: PropTypes.bool, // Добавляем проверку типа для isActive
}
